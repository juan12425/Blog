...
function deletetext($conn, $title)
{
    ...
    if(!unlink($path))
    {
        ...
        header('location: texts2.php?=eliminaciónexitosa');
        exit();
    }
}
...
function createtopic($conn, $title, $description)
{
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("location: Ágora.php?normal&error=none");
    exit();
}
...
function gettopics($conn)
{
    ...
    if(!mysqli_stmt_prepare($stmt, $sql))
    {
        header("location: Ágora.php?normal&error=stmtfallidot");   
        exit();
    }
...
function createthread($conn, $title, $answer, $responsable, $responsable_id, $topic, $range, $date)
{
    ...
    if(!mysqli_stmt_prepare($stmt, $sql))
    {
        header("location: Ágora.php?topic=".$topic."&action=thread&error=stmtfallido");   
        exit();
    }
    mysqli_stmt_bind_param($stmt, "sssisss", $title, $answer, $responsable, $responsable_id, $topic, $range, $date);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("location: Ágora.php?topic=".$topic."&error=none");
    exit();
...
function getthreads($conn, $topic)
{
    ...
    if(!mysqli_stmt_prepare($stmt, $sql))
    {
        header("location: Ágora.php?topic=".$topic."&error=stmtfallidot");   
        exit();
    }
...
function getanswers($conn, $id)
{
    ...
    if(!mysqli_stmt_prepare($stmt, $sql))
    {
        header("location: Ágora.php?topic=".$topic."&error=stmtfallidocargandorespuestas");   
        exit();
    }
...
function updatetopictitle($conn, $newtitle, $oldtitle)
{
    ...
    header("location: Ágora.php?error=none");
    exit();
}
...
function deletethread($conn, $title)
{
    ...
    if(!mysqli_stmt_prepare($stmt, $sql))
    {
        header("location: Ágora.php?normal&error=stmtfallidoeliminandohilo");   
        exit();
    }
...
function  edittethread($conn, $id, $newtitle, $answer)
{
    ...
    if(!mysqli_stmt_prepare($stmt, $sql))
    {
        header("location: Ágora.php?error=stmtfallidotaleditarhilo");   
        exit();
    }
...
function deleteanswer($conn, $id)
{
    ...
    if(!mysqli_stmt_prepare($stmt, $sql))
    {
        header("location: Ágora.php?normal&error=stmtfallidoeliminandorespuesta");   
        exit();
    }
...
function updateanswer($conn, $id, $answer)
{
    ...
    if(!mysqli_stmt_prepare($stmt, $sql))
    {
        header("location: Ágora.php?error=stmtfallidotaleditarrespuesta");   
        exit();
    }
