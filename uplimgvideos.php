...
if(isset($_POST['submit'])  ) {
    ...
    if($status=="set")
    {
        header("Location: videos.php?=eliminelaimagenantesdesubirotra");
        exit();
    }
        ...
        if( $file_error===0){
            if($file_size < 10000000){
                ...
                $file_newlocation='videos/'.$file_newname;

                move_uploaded_file($file_location, $file_newlocation);
                
                setset3($conn, $file_newname, $sname); //
                
                header("Location: videos.php?=cargaexitosa");
